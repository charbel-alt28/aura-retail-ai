...
export function PricingSalesControl() {
  ...
  const handleOptimizePrices = () => {
    if (!hasPermission('modify_prices')) {
  ...
  const handleLaunchPromotion = () => {
    if (!hasPermission('launch_promotion')) {
      toast.error('Insufficient permissions to launch promotions');
      return;
    }
    const slowItems = products.filter(p => p.demandLevel === 'low');
    setConfirmAction({
      title: 'Launch Discount Campaign',
      description: `This will apply a 15% discount to ${slowItems.length} slow-moving items. Current prices will be permanently reduced. Continue?`,
      onConfirm: () => {
        slowItems.forEach(p => applyPromotion(p.id, 15));
        addAgentLog({ agent: 'pricing', action: 'PROMO_LAUNCH', details: `Launched 15% discount campaign on ${slowItems.length} slow-moving items`, status: 'info' });
                ...
                >
                  <div className="flex items-center gap-1.5 w-full">
                    ...
                    {op.badge != null && op.badge > 0 && (
                      <span className="ml-auto text-[9px] bg-warning/20 text-warning px-1.5 rounded-full font-bold">{op.badge}</span>
                    )}
                  </div>
                        ...
                        return (
                          <div key={item.id} className="flex items-center justify-between bg-muted/20 rounded px-2 py-1.5 text-[11px]">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className={cn("font-bold text-[10px] px-1.5 rounded", margin < 10 ? 'status-critical' : 'status-warning')}>{margin.toFixed(0)}%</span>
                            </div>
                          </div>
                        );
                      })}
